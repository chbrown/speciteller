...
def mpqaGenInqInfo(sentlst):
    ...
    for t in sentlst:
        ...
        for nodetxt in t.getTokens():
            txt = nodetxt.lower()
            if txt in mpqa:
                if mpqa[txt].subj == "strongsubj":
                    s += 1
                if mpqa[txt].polarity != "neutral":
                    p += 1
                if mpqa[txt].polarity == "negative":
                    ng += 1
                if mpqa[txt].polarity == "positive":
                    ps += 1
            elif txt in geninq:
                if geninq[txt][2] == 1:
                    s += 1
                if geninq[txt][0] + geninq[txt][1] > 0:
                    p += 1
                if geninq[txt][0] == 1:
                    ps += 1
                if geninq[txt][1] == 1:
...
class Space:
    ...
    def __repr__(self):
        ...
        if self.featselset is not None:
            s += "selected features: " + str(len(self.featselset)) + "\n"
        return s
    ...
    def toFeatStr(self, lst, is_binary, minfreq=3):
        s = defaultdict(int)
        for e in lst:
            if e not in self.f_id or self.freq[e] < minfreq or (self.featselset is not None and e not in self.featselset):
                continue
